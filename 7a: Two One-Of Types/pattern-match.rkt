...
;; produces true of ListOfOneStrings matches the pattern
;; (define (match? p ls) false) ;Stub
...
(check-expect (match? empty (list "K")) true)
(check-expect (match? empty (list "1")) true)
(check-expect (match? (list "A") (list "K")) true)
(check-expect (match? (list "N") (list "K")) false)
(check-expect (match? (list "A") (list "0")) false)
(check-expect (match? (list "N") (list "0")) true)
(check-expect (match? (list "A" "N") (list "K" "0")) true)
(check-expect (match? (list "A" "N") (list "0" "N")) false)
(check-expect (match? (list "A" "N" "A" "A") (list "K" "1" "A" "M" "R" "A" "N")) true)
(check-expect (match? (list "A" "N" "A" "A") (list "K" "A" "A" "M" "R" "A" "N")) false)
(check-expect (match? (list "A" "N" "A" "A" "N") (list "K" "1" "A" "M" "A" "A" "N")) false) 

(define (match? p l)
  (cond [(empty? p) true]
        [(empty? l) false]
        [(string=? (first p) "A") (and (alphabetic? (first l)) (match? (rest p) (rest l)))]
        [(string=? (first p) "N") (and (numeric? (first l)) (match? (rest p) (rest l)))]))
